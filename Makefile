...
.PHONY: help venv deps test lint typecheck policy check-policy check \
        agent-chat agent-tools agent-doctor overlay image vm clean
...
help:
	...
	@echo "  make check         test + lint + cohérence de la politique"
...
clean:
	rm -rf .pytest_cache .mypy_cache out
